/*
  MURMUR servo bridge
  ----------------------
  Bidirectional bridge between MURMUR (index.html) running in Chrome/Edge
  and a row of hobby servos, over WebSerial.

  BROWSER -> ARDUINO (servo positions)
    MURMUR samples the live sphere field (whatever mode is active — ambient,
  ...
  ARDUINO -> BROWSER (sensor feedback, closes the loop)
    ...
    e.g. "S612\n". In MURMUR, set "Sensor drives" (in the Physical rig panel)
  ...
  4. (Optional, for sensor feedback) Wire SENSOR_PIN (default A0) to either:
     ...
     leave "Sensor drives" set to Off in MURMUR if you haven't wired one up.
  5. Upload this sketch, then open MURMUR in Chrome/Edge and click "Connect
  ...
  6. (Optional) Open the Arduino IDE's Serial Monitor at 115200 baud before
     connecting from the browser — you should see "MURMUR-SERVO-READY" printed
...
const unsigned long SENSOR_SEND_INTERVAL_MS = 50; // ~20Hz, matches MURMUR's own send rate
...
void setup() {
  ...
  lineBuf.reserve(128);
  Serial.println("MURMUR-SERVO-READY");
}

...
A field of 968 kinetic spheres that drift through noise and resolve into text or shapes — inspired by ART+COM's BMW Kinetic Sculpture at the BMW Museum.
...
FORMFIND is built on these open-source projects:

- [Three.js](https://threejs.org) (MIT) — all 3D rendering, both the kinetic sphere field and the simulated servo rig previews.
- [avr8js](https://github.com/wokwi/avr8js) (MIT) by Wokwi — runs the real compiled `formfind_servo.ino` firmware directly, in-browser (via the embedded preview's Web Worker) and in the standalone `avr8js_sim_bridge.js` bridge script, rather than approximating servo behavior.
